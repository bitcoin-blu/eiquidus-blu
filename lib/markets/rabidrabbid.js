...
function get_summary(coin, exchange, api_error_msg, cb) {
  ...
  const pair_key = coin + '_' + exchange;
    ...
    request({uri: req_url, json: true}, function (error, response, body) {
      if (error)
        ...
        try {
          ...
          if (!body[pair_key]) {
            return cb('Pair ' + pair_key + ' not found', null);
          }
          
          const pair_data = body[pair_key];
          const summary = {
            'high': 0, // Not available in this API
            'low': 0,  // Not available in this API
            'volume': parseFloat(pair_data.base_volume) || 0,
            'volume_btc': parseFloat(pair_data.quote_volume) || 0,
            'bid': 0,  // Not available in this API
            'ask': 0, // Not available in this API
            'last': parseFloat(pair_data.last_price) || 0,
            'prev': 0, // Not available in this API
            'change': 0 // Not available in this API
          };
...
function get_trades(coin, exchange, api_error_msg, cb) {
  // Rabid Rabbit API doesn't provide historical trades endpoint
  // Return empty array as this data is not available
  return cb(null, []);
}

function get_orders(coin, exchange, api_error_msg, cb) {
  // Rabid Rabbit API doesn't provide order book endpoint
  // Return empty arrays as this data is not available
  return cb(null, [], []);
}

function get_chartdata(coin, exchange, api_error_msg, cb) {
  // Rabid Rabbit API doesn't provide chart data endpoint
  // Return empty array as this data is not available
  return cb(null, []);
}
